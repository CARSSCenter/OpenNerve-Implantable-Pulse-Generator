...
void bsp_sp_deinit(void) {
	...
	if (init) {
		...
		//HAL_SPI_MspDeInit(&HANDLE_DAC80502_SPI);
...
/**
 * @brief Write data to DAC80501 on serial port
 *
 * @param reg_addr The address of the DAC80501 register
 * @param reg_data The data to be written to the DAC80501 register
 * @return uint8_t HAL status
 */
__weak uint8_t bsp_sp_DAC80501_write(uint8_t reg_addr, uint8_t* reg_data) {
	return (uint8_t)HAL_I2C_Mem_Write(&HANDLE_ISL23315T_DAC8050x_I2C, BSP_DAC80501_DEVICE_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, reg_data, 2, 5);
}

/**
 * @brief Read data from DAC80501 on serial port
 *
 * @param reg_addr The address of the DAC80501 register
 * @param reg_data Data read from DAC80501 register
 * @return uint8_t HAL status
 */
__weak uint8_t bsp_sp_DAC80501_read(uint8_t reg_addr, uint8_t* reg_data) {
	return (uint8_t)HAL_I2C_Mem_Read(&HANDLE_ISL23315T_DAC8050x_I2C, BSP_DAC80501_DEVICE_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, reg_data, 2, 5);
}

/**
 * @brief Write data to DAC80501 on serial port in non-blocking mode
 *
 * @param reg_addr The address of the DAC80501 register
 * @param reg_data The data to be written to the DAC80501 register
 */
__weak void bsp_sp_DAC80501_write_IT(uint8_t reg_addr, uint8_t* reg_data) {
	DAC8050x_reg_addr = reg_addr;
	(void)memcpy((uint8_t*)DAC8050x_reg_data, reg_data, 2);
	(void)HAL_I2C_Mem_Write_IT(&HANDLE_ISL23315T_DAC8050x_I2C, BSP_DAC80501_DEVICE_ADDR, DAC8050x_reg_addr, I2C_MEMADD_SIZE_8BIT, DAC8050x_reg_data, 2);
}
...
uint8_t bsp_sp_MIS2DHTR_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t* reg_data, uint16_t data_len) {
	return (uint8_t)HAL_I2C_Mem_Write(&HANDLE_ACC_I2C, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, reg_data, data_len, 5);
}
...
uint8_t bsp_sp_MIS2DHTR_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t* reg_data, uint16_t data_len) {
	return (uint8_t)HAL_I2C_Mem_Read(&HANDLE_ACC_I2C, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, reg_data, data_len, 5);
}
