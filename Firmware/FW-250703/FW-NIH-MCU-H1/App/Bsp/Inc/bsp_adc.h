...
#define ADC1_CHANNEL_EMG1_OUT			ADC_CHANNEL_17	//PB2
...
#define ADC4_CHANNEL_EMG2_OUT			ADC_CHANNEL_7	//PG0
 ...
 */
void bsp_adc_single_sampling(uint8_t hadcID, uint32_t channel, uint16_t voltageBuffer[], uint16_t samplingPoints, uint16_t samplingFrequency_hz, uint32_t vrefanalog_mv);
 ...
 * @param vrefanalog_mv Vref of the ADC in mV
 */
void bsp_adc_differential_sampling(uint8_t hadcID, uint32_t ch1, uint32_t ch2, uint16_t voltageBuffer[], uint16_t samplingPoints, uint16_t samplingFrequency_hz, uint32_t vrefanalog_mv);
