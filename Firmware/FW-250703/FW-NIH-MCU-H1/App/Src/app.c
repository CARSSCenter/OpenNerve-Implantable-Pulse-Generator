...
{
	if (htim == &HANDLE_STIMULATION_TIM && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
		app_func_stim_stimulus_cb(false);
	}
}
...
{
	if (htim == &HANDLE_STIMULATION_TIM) {
		app_func_stim_VNSb_cb();
		app_func_stim_stimulus_cb(true);
	}
...
void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim == &HANDLE_STIMULATION_TIM) {
		if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_4) {
			app_func_stim_sine_cb();
		}
	}
	else if (htim == &HANDLE_FORCE_PH_OUT_TIM) {
		app_func_stim_ph_out_force_cb();
	}
}
